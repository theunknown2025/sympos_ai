...
import { 
  ...
  Clock, FileText, Mail, Phone, User, Briefcase, Tag
} from 'lucide-react';
import { getEvent } from '../../../services/eventService';
import { getCommittee } from '../../../services/committeeService';
import { getOrganizerProfile } from '../../../services/organizerProfileService';
import { Event, Committee, EventBanner, OrganizerProfile } from '../../../types';
...
const EventPreview: React.FC = () => {
  ...
  const loadEvent = async () => {
    try {
      ...
      if (eventData.userId) {
        try {
          const profile = await getOrganizerProfile(eventData.userId);
          setOrganizerProfile(profile);
        } catch (err) {
          console.error('Error loading organizer profile:', err);
          // Don't fail the whole page if organizer profile fails to load
        }
      }
    } catch (err: any) {
          ...
          <div className="space-y-6">
            ...
            {organizerProfile && (
              <section className="bg-white rounded-lg shadow-sm p-6">
                <div className="flex items-center gap-2 mb-4">
                  <Building className="w-5 h-5 text-indigo-600" />
                  <h2 className="text-lg font-semibold text-slate-800">Organizer</h2>
                </div>
                <div className="space-y-3">
                  {organizerProfile.entityLogo && (
                    <div className="mb-3">
                      <img 
                        src={organizerProfile.entityLogo} 
                        alt={organizerProfile.entityName || 'Organizer logo'} 
                        className="h-16 w-auto object-contain"
                      />
                    </div>
                  )}
                  {organizerProfile.entityName && (
                    <div>
                      <p className="font-semibold text-slate-800">{organizerProfile.entityName}</p>
                    </div>
                  )}
                  {organizerProfile.entityEmail && (
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      <Mail size={14} />
                      <a href={`mailto:${organizerProfile.entityEmail}`} className="hover:text-indigo-600">
                        {organizerProfile.entityEmail}
                      </a>
                    </div>
                  )}
                  {organizerProfile.entityPhone && (
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      <Phone size={14} />
                      <a href={`tel:${organizerProfile.entityPhone}`} className="hover:text-indigo-600">
                        {organizerProfile.entityPhone}
                      </a>
                    </div>
                  )}
                  {organizerProfile.entityAddress && (
                    <div className="flex items-start gap-2 text-sm text-slate-600">
                      <MapPin size={14} className="mt-0.5 flex-shrink-0" />
                      <span>{organizerProfile.entityAddress}</span>
                    </div>
                  )}
                  {organizerProfile.entityWebsites && organizerProfile.entityWebsites.length > 0 && (
                    <div className="space-y-1">
                      {organizerProfile.entityWebsites.map((website, index) => (
                        <a
                          key={index}
                  ...
                  {organizerProfile.entityLinks && organizerProfile.entityLinks.length > 0 && (
                    <div className="space-y-1 pt-2 border-t border-slate-200">
                      {organizerProfile.entityLinks.map((link) => (
                        <a
                          key={link.id}
                          href={link.url}
                          target="_blank"
            ...
            {organizerProfile && (organizerProfile.representativeFullName || organizerProfile.representativeEmail || organizerProfile.representativePhone) && (
              <section className="bg-white rounded-lg shadow-sm p-6">
                <div className="flex items-center gap-2 mb-4">
                  <User className="w-5 h-5 text-indigo-600" />
                  <h2 className="text-lg font-semibold text-slate-800">Representative</h2>
                </div>
                <div className="space-y-3">
                  {organizerProfile.representativeFullName && (
                    <div>
                      <p className="font-semibold text-slate-800">{organizerProfile.representativeFullName}</p>
                    </div>
                  )}
                  {organizerProfile.representativeFunction && (
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      <Briefcase size={14} />
                      <span>{organizerProfile.representativeFunction}</span>
                    </div>
                  )}
                  {organizerProfile.representativeEmail && (
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      <Mail size={14} />
                      <a href={`mailto:${organizerProfile.representativeEmail}`} className="hover:text-indigo-600">
                        {organizerProfile.representativeEmail}
                      </a>
                    </div>
                  )}
                  {organizerProfile.representativePhone && (
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      <Phone size={14} />
                      <a href={`tel:${organizerProfile.representativePhone}`} className="hover:text-indigo-600">
                        {organizerProfile.representativePhone}
                      </a>
                    </div>
                  )}
                  {organizerProfile.representativeAddress && (
                    <div className="flex items-start gap-2 text-sm text-slate-600">
                      <MapPin size={14} className="mt-0.5 flex-shrink-0" />
                      <span>{organizerProfile.representativeAddress}</span>
                    </div>
                  )}
                </div>
              </section>
