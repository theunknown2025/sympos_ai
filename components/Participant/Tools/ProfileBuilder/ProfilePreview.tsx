...
const ProfilePreview: React.FC<ProfilePreviewProps> = ({ profile }) => {
  const [activeTab, setActiveTab] = useState<string>(
    profile.sections.length > 0 ? profile.sections[0].id : ''
  );
          ...
          <div className="space-y-4">
            {section.data.publications?.map((pub: any, index: number) => (
              <div key={index} className="border-l-2 border-indigo-500 pl-4 py-2">
                ...
                {pub.date && <p className="text-sm text-slate-500 mt-1" style={designStyles.dates}>{pub.date}</p>}
                {pub.description && (
  ...
  if (template !== 'default') {
    return <ProfileTemplate profile={profile as any} />;
  }
  ...
  return (
    <div
      className="h-full flex flex-col bg-white rounded-lg shadow-sm border border-slate-200"
      style={designStyles.background}
      dir={direction}
    >
      <div
        className="p-6 border-b border-slate-200"
        style={designStyles.banner}
      >
        <h2 className="text-lg font-semibold text-slate-900" style={designStyles.titles}>Preview</h2>
      </div>
