...
import { Save, X, Loader2, AlertCircle, Image as ImageIcon } from 'lucide-react';
...
const NewArticle: React.FC<NewArticleProps> = ({ articleId, onSave, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const articleData: Omit<BlogArticle, 'id' | 'userId' | 'createdAt' | 'updatedAt'> = {
        title: title.trim(),
      ...
      if (articleId) {
        ...
        await saveBlogArticle(currentUser.id, articleData);
      }
      onSave();
    } catch (err: any) {
