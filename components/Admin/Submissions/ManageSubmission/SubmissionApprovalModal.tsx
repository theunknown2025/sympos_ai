...
interface SubmissionApprovalModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (approval: ApprovalStatus, comment: string) => Promise<void>;
  currentApproval?: ApprovalStatus;
  currentComment?: string;
}

const SubmissionApprovalModal: React.FC<SubmissionApprovalModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  currentApproval,
  ...
  }, [isOpen, currentApproval, currentComment]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError('');
      await onSubmit(approval, comment);
      onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-200">
          <h2 className="text-2xl font-bold text-slate-900">Submission Approval</h2>
          <button
            onClick={handleClose}
