...
const PricingEditor: React.FC<PricingEditorProps> = ({ pricing, onChange }) => {
  ...
  const addOffer = () => {
    const newOffer: PricingOffer = {
      ...
      name: t('edPricingNewTicketType'),
      price: '0',
      currency: '$',
      features: [],
      buttonText: t('edPricingRegisterNow'),
      buttonUrl: '#',
      isSoldOut: false,
      isHighlighted: false,
    };
  ...
  const addFeature = (offerId: string) => {
    ...
    if (offer) {
      updateOffer(offerId, 'features', [...offer.features, t('edPricingNewFeature')]);
    }
  };
  ...
  const removeFeature = (offerId: string, idx: number) => {
    ...
    if (offer) {
      updateOffer(
        offerId,
        'features',
        offer.features.filter((_, i) => i !== idx)
      );
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-3">
        <span className="text-xs text-slate-500">{t('edPricingOffersSummary', { count: pricing.length })}</span>
        <button onClick={addOffer} className="text-xs flex items-center gap-1 text-indigo-600 font-medium hover:text-indigo-800">
          <Plus size={12} /> {t('edPricingAddOffer')}
        </button>
      </div>
          ...
          <div key={offer.id} className="border border-slate-200 rounded-lg overflow-hidden bg-slate-50">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h5 className="text-sm font-semibold text-slate-800">{offer.name || t('edPricingUnnamedOffer')}</h5>
              ...
              <div className="p-4 border-t border-slate-100 space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <input
                    ...
                    placeholder={t('edPricingOfferNamePlaceholder')}
                  ...
                  <input
                    ...
                    placeholder={t('edPricingPricePlaceholder')}
                    className="w-full px-2 py-1.5 border border-slate-200 rounded text-sm bg-white"
                  />
                </div>
                <div className="flex items-center justify-between p-2 bg-white border border-slate-200 rounded-lg">
                  <span className="text-xs font-medium text-slate-700">{t('edPricingHighlighted')}</span>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      className="sr-only peer"
                      checked={offer.isHighlighted}
                      onChange={(e) => updateOffer(offer.id, 'isHighlighted', e.target.checked)}
                    />
                ...
                <div className="space-y-2">
                  {offer.features.map((f, i) => (
                    <div key={i} className="flex gap-2">
                      ...
                      />
                      <button onClick={() => removeFeature(offer.id, i)} className="text-slate-300 hover:text-red-500">
                        <X size={14} />
                      </button>
                    </div>
                  ))}
                  <button onClick={() => addFeature(offer.id)} className="text-[10px] text-indigo-600 font-medium">
                    {t('edPricingAddFeature')}
                  </button>
                </div>
              </div>
            )}
