...
import { Plus, Trash2, ChevronUp, ChevronDown, X, Download, Loader2, AlertCircle } from 'lucide-react';
...
const AgendaEditor: React.FC<AgendaEditorProps> = ({
  ...
  const loadPrograms = async () => {
    ...
    try {
      ...
      setError(err.message || t('edAgendaFailedLoadPrograms'));
  ...
  const addDay = () => {
    const newDay: AgendaDay = {
      ...
      label: t('edAgendaDayNumber', { n: agenda.length + 1 }),
      date: '',
      items: [],
    };
  ...
  const addItem = (dayId: string) => {
    ...
    if (day) {
      const newItem: AgendaItem = {
        ...
        startTime: '09:00',
        endTime: '10:00',
        title: t('edAgendaNewActivityDefault'),
        description: '',
        location: '',
      };
  ...
  const removeItem = (dayId: string, itemId: string) => {
    ...
    if (day) {
      updateDay(
        dayId,
        'items',
        day.items.filter((i) => i.id !== itemId)
      );
    }
  };
            ...
            >
              <div>
                ...
                <span className="text-xs text-slate-500">
                  {day.date || t('edAgendaSetDate')} • {t('edAgendaActivitiesCount', { count: day.items.length })}
                </span>
              </div>
              ...
              <div className="p-4 border-t border-slate-100 space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <input
                    ...
                    placeholder={t('edAgendaDayLabelPlaceholder')}
                  />
                  <input
                    type="date"
                    value={day.date}
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    <label className="block text-xs font-bold text-slate-500 uppercase">{t('edAgendaActivitiesLabel')}</label>
                    <button onClick={() => addItem(day.id)} className="text-[10px] text-indigo-600 font-medium hover:underline">
                      {t('edAgendaAddActivity')}
                    </button>
                  </div>
                    ...
                    {day.items.map((item) => (
                      <div key={item.id} className="bg-white p-3 rounded border border-slate-200 space-y-2 relative group">
                        ...
                        <input
                          ...
                          placeholder={t('edAgendaItemTitlePlaceholder')}
                        ...
                        <select
                          ...
                          onChange={(e) => updateItem(day.id, item.id, 'speakerId', e.target.value)}
                          className="w-full px-2 py-1 border border-slate-200 rounded text-xs bg-white text-slate-900"
                        >
                          <option value="">{t('edAgendaNoSpeaker')}</option>
                          {speakers.map((s) => (
      ...
      {onProgramConfigChange && (
        <div className="border border-slate-200 rounded-lg p-4 bg-blue-50/50 space-y-3 mt-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <label className="text-xs font-bold text-slate-700 uppercase">{t('edAgendaProgramDownload')}</label>
          </div>

          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">{t('edAgendaConnectProgram')}</label>
            {loadingPrograms ? (
              <div className="flex items-center gap-2 text-xs text-slate-500">
                <Loader2 className="animate-spin" size={12} />
                {t('edAgendaLoadingPrograms')}
              </div>
            ) : error ? (
              ...
              <div className="text-xs text-slate-500 p-2 bg-slate-50 rounded border border-slate-200">{t('edAgendaNoProgramsHint')}</div>
            ) : (
              <select
                value={programId || ''}
              ...
              >
                <option value="">{t('edAgendaNoProgramSelected')}</option>
          ...
          {programId && (
            <div>
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-xs text-slate-700">{t('edAgendaShowDownloadButton')}</span>
              </label>
              <p className="text-[10px] text-slate-500 mt-1 ml-5">{t('edAgendaDownloadPdfHelp')}</p>
            </div>
          )}
        </div>
      )}
