...
import { Plus, Trash2, Users, ChevronUp, ChevronDown } from 'lucide-react';
...
const PartnersEditor: React.FC<PartnersEditorProps> = ({ partners, onChange }) => {
  ...
  const addGroup = () => {
    const newGroup: PartnerGroup = { id: Date.now().toString(), name: t('edPartnersNewGroup'), displayStyle: 'grid', partners: [] };
  ...
  const addLogo = (groupId: string) => {
    ...
    if (group) {
      const newPartner: Partner = { id: Date.now().toString(), name: t('edPartnersLogoName'), logoUrl: '' };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-3">
        <span className="text-xs text-slate-500">{t('edPartnersGroupsSummary', { count: partners.length })}</span>
        <button onClick={addGroup} className="text-xs flex items-center gap-1 text-indigo-600 font-medium hover:text-indigo-800">
          <Plus size={12} /> {t('edPartnersAddGroup')}
        </button>
      </div>
            ...
            {expandedId === group.id && (
              <div className="p-4 border-t border-slate-100 space-y-4">
                <input
                  ...
                  placeholder={t('edPartnersGroupNamePlaceholder')}
                  className="w-full px-2 py-1.5 border border-slate-200 rounded text-sm bg-white"
                />
                <div className="flex justify-between items-center">
                  <label className="text-[10px] font-bold text-slate-500 uppercase">{t('edPartnersLogos', { count: group.partners.length })}</label>
                  <button onClick={() => addLogo(group.id)} className="text-[10px] text-indigo-600 font-medium">
                    {t('edPartnersAdd')}
                  </button>
                </div>
              </div>
