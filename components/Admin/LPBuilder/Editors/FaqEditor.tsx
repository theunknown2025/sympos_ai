...
const FaqEditor: React.FC<FaqEditorProps> = ({ faq, onChange }) => {
  ...
  const addFaq = () => {
    const newItem: FaqItem = { id: Date.now().toString(), question: t('edFaqNewQuestionDefault'), answer: '', icon: 'help' };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <span className="text-xs text-slate-500">{t('edFaqQuestionsSummary', { count: faq.length })}</span>
        <button onClick={addFaq} className="text-xs flex items-center gap-1 text-indigo-600 font-medium hover:text-indigo-800">
          <Plus size={12} /> {t('edFaqAddQuestion')}
        </button>
      </div>
          ...
          <div key={item.id} className="border border-slate-200 rounded-lg overflow-hidden bg-slate-50">
            ...
            >
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                <p className="text-sm font-semibold text-slate-800 truncate">{item.question || t('edFaqNewQuestionFallback')}</p>
              </div>
