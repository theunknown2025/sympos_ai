            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center overflow-hidden shrink-0">
                  {speaker.imageUrl ? (
                    <img src={speaker.imageUrl} alt="" className="w-full h-full object-cover" />
                  ) : (
                    <User size={16} className="text-indigo-400" />
                  )}
                </div>
                <div className="overflow-hidden">
                  <p className="text-sm font-semibold text-slate-800 truncate">{speaker.name || t('edDefaultNewSpeaker')}</p>
                  <p className="text-xs text-slate-500 truncate">{speaker.role || t('edNoTitleYet')}</p>
                </div>
              </div>
            ...
            {expandedId === speaker.id && (
              <div className="p-4 border-t border-slate-100 space-y-3">
                ...
                <input
                  ...
                  placeholder={t('edPlaceholderTitleRole')}
                  className="w-full px-2 py-1.5 border border-slate-200 rounded text-sm bg-white text-slate-900"
                />
                ...
                <textarea
                  ...
                  placeholder={t('edPlaceholderBiography')}
                  className="w-full px-2 py-1.5 border border-slate-200 rounded text-sm resize-none bg-white text-slate-900"
                />
                <div>
                  <button onClick={() => addSocial(speaker.id)} className="text-[10px] text-indigo-600 font-medium hover:underline">
                    {t('edAddSocialLink')}
                  </button>
                </div>
              </div>
