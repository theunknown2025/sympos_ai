...
const TABLE_NAME = TABLES.BADGE_TEMPLATES;
...
/**
 * Save a new badge template
 */
export const saveBadgeTemplate = async (
  userId: string,
 ...
 */
export const getBadgeTemplate = async (templateId: string): Promise<CertificateTemplate | null> => {
  try {
    const { data, error } = await supabase
      .from(TABLE_NAME)
      .select('*')
...
/**
 * Get all badge templates for a user
 */
export const getUserBadgeTemplates = async (userId: string): Promise<CertificateTemplate[]> => {
  try {
    const { data, error } = await supabase
      .from(TABLE_NAME)
      .select('*')
    ...
    console.error('Error getting user badge templates:', error);
    throw error;
  }
};
 ...
 */
export const deleteBadgeTemplate = async (templateId: string): Promise<void> => {
  try {
    const { error } = await supabase
      .from(TABLE_NAME)
      .delete()
    ...
    throw new Error(error.message || 'Failed to delete badge template');
  }
};
