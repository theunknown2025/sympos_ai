...
const TABLE_NAME = TABLES.EVALUATION_FORMS;
...
/**
 * Save a new evaluation form
 */
export const saveEvaluationForm = async (
  userId: string,
  form: Omit<EvaluationForm, 'id' | 'userId' | 'createdAt' | 'updatedAt'>
): Promise<string> => {
  try {
    if (!userId) {
...
export const updateEvaluationForm = async (
  formId: string,
  form: Partial<Omit<EvaluationForm, 'id' | 'userId' | 'createdAt' | 'updatedAt'>>
): Promise<void> => {
  try {
    const cleanedForm = removeUndefined(form);
 ...
 */
export const getEvaluationForm = async (formId: string): Promise<EvaluationForm | null> => {
  try {
    const { data, error } = await supabase
      .from(TABLE_NAME)
      .select('*')
    ...
    console.error('Error getting evaluation form:', error);
    throw error;
  }
};

/**
 * Get all evaluation forms for a user
 */
export const getUserEvaluationForms = async (userId: string): Promise<EvaluationForm[]> => {
  try {
    if (!userId) {
      throw new Error('User ID is required');
    }
    ...
    console.error('Error getting user evaluation forms:', error);
    throw error;
  }
};
 ...
 */
export const deleteEvaluationForm = async (formId: string): Promise<void> => {
  try {
    const { error } = await supabase
      .from(TABLE_NAME)
      .delete()
    ...
    throw new Error(error.message || 'Failed to delete evaluation form');
  }
};
